...
The proposed method is an industrial decision framework for selecting deployable planning strategies. Figure~\ref{fig:framework} shows the four layers. Candidate forecasts and policies are first translated into planning signals. The same feasibility scoring layer then evaluates every candidate before the decision layer ranks deployable strategies. This design follows forecast-to-decision research showing that downstream operational quality can differ from forecast-error quality (Elmachtoub and Grigas 2022; Mandi et al. 2024).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\linewidth]{figures/ieom_framework_flowchart.pdf}
\caption{Four-layer framework for feasibility-constrained demand planning.}
\label{fig:framework}
\end{figure}

Figure~\ref{fig:framework} separates diagnostic forecast accuracy from deployable planning utility. It also links method components to validation evidence: DataCo stress scenarios test execution pressure, DP and oracle benchmarks test decision-layer value, M5 tests hierarchy and intermittency, and Walmart tests business context and weekly planning cadence.
...
Steps 1--3 generate candidate planning signals. Steps 4--6 compute inventory exposure, planning volatility, execution violation, and planning-execution gap rate. Step 7 produces deployable strategy rankings. Step 8 validates those rankings using DataCo scenarios, DP-oracle comparison, M5 hierarchy and intermittency robustness, and Walmart business-context and cadence robustness. The procedure is modular: accuracy-first baselines, ensembles, smoothing rules, greedy selectors, DP selectors, and budgeted-DP selectors all pass through the same feasibility layer.

Table~\ref{tab:method-alignment} links the framework components to the evidence used in this paper and to their operational use.
