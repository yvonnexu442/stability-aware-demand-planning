...
\item \textbf{Governance burden}: Each model change requires validation, approval, and monitoring cycles, delaying rollout
...
The remaining sections follow the same arc as the four symptoms in Section~\ref{sec:gap}: Section~\ref{sec:methods} turns the feasibility gate into a mechanism, Section~\ref{sec:results} reports how often and why accuracy-first and deployment-best selection diverge, Section~\ref{sec:protocol} converts that evidence into a five-step deployment protocol, and Section~\ref{sec:validation} closes the loop by testing whether the protocol holds under tighter execution risk, coarser and finer planning grains, and a second dataset with a different cadence and business context.
